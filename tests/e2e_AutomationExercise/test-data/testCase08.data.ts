export const testCase08Data = {
  urlProductDetails: 'https://automationexercise.com/product_details/1',
  verifyHomePage: 'Automation Exercise',
  messageProducts: 'ALL PRODUCTS',
  messageProductList:
    'All Products  Added! Your product has been added to cart. View Cart Continue Sh',
  messageProductDetails:
    'Blue Top Category: Women > Tops Rs. 500 Quantity: Add to cart Availability: In S',
};
