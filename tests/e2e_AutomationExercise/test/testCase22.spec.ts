...
test.describe('Test Case 22: Add to cart from Recommended items', () => {
  test('TC22 POM add to cart from recommended items', async ({ page }) => {
    ...
    const verifyProductIsDisplays = testCase22Data.verifyProductIsDisplays;
    ...
    await page
      .locator('div:nth-child(2) > .product-image-wrapper > .single-products > .productinfo > .add-to-cart')
      .first()
      .click();
    await page.getByRole('link', { name: 'View Cart' }).click();
    await expect(page.getByRole('row', { name: verifyProductIsDisplays })).toBeVisible();
  });
});
